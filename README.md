...
Модуль содержит глобальную переменную `DMpy.universum` типа `Carrier.Carrier`, содержащую универсум рассматриваемой задачи. Все функции, в которые передаются данные из универсума, должны работать не с самими данными, а с их отображениями, получаемыми с помощью универсума. Но каждый метод, принимающий на вход элементы универсума, имеет также аргумент `images = False`, который, если равен `True`, означает, что переданные данные уже преобразованы в числа и второй раз преобразовывать их не нужно.

`DMpy` содержит метод `DMpy.setUniversum( iterable )`, устанавливающий универсум текущей задачи. Он должен проинициализировать  `DMpy.universum` значением  `DMpy.Carrier.Carrier( iterable )`. 



### 3.1 Носитель ( =*Carrier* )

Модуль содержит класс `Carrier`. Он нужен для того, чтобы преобразовывать фиксированный набор объектов в числа на отрезок 1 ..n и обратно.
 ( на начальном этапе )

**Конструкторы:**

* `Carrier( iterable )`  :`iterable` - любой перечислимый объект - заданное рабочее множество.
...
### 3.2 Подмножества и семейства ( =*Sets* )
...
### 3.3 Одноместные функции ( =*Functions* )
...
### 3.4 Двуместные операции ( =*Operations* )
...
### 3.5 Визуализация ( =*Visualization* )
...
### 3.6 Сериализация ( =*Serialization* )
