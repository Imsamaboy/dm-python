...
Модуль содержит глобальную переменную `DMPy.universum` типа `Support.Support`, нужную для отображения данных, переданных в функции и методы пакета, в числа. Все функции и методы должны работать не с самими данными, а с их отображениями, получаемыми с помощью этого объекта. Это преобразование осуществляет пользователь.
...
* `__str__()` преобразующий множество в строку для возможности вывода через print
* `cup(set)` возвращает объединение с переданным множеством
* `cap(set)` возвращает пересечение с переданным множеством
* `equals(set)` определяет, равно ли множество переданному
